...
export const PopulationGraph = () => {
  // Your actual data
  const data = [
    { ageGroup: "65+", male: "0.66%", female: "0.01%" },
    { ageGroup: "55-64", male: "8.41%", female: "1.88%" },
    { ageGroup: "45-54", male: "10.28%", female: "4.37%" },
    { ageGroup: "35-44", male: "15.86%", female: "9.84%" },
    { ageGroup: "25-34", male: "20.01%", female: "12.67%" },
    { ageGroup: "15-24", male: "6.72%", female: "9.28%" },
  ];
  ...
  const tooltipValue = Math.round(
    parseFloat(tooltip.value?.split("%")?.[0]) * 84.92
  ).toFixed(0);
        ...
        <div className="relative bg-white border-2 border-gray-300 overflow-hidden">
          <div className="flex flex-col relative z-10">
            {data.map((item, index) => (
              <div
                key={index}
                className="flex items-center self-center border-b border-gray-400 "
              >
                {/* Male side */}
                <div className="flex flex-col  justify-end items-center pr-0 relative">
                  <div
                  ...
                  >
                    ...
                    <div
                      className="bg-blue-500 border-r border-blue-700 h-6 flex items-center justify-start pl-1 relative"
                  ...
                  >
                    <span className="text-sm">
                      {parseFloat(data2020[index].male).toFixed(2)}%
                    </span>
                    <div
                      className="bg-blue-500 border-r border-blue-700 h-6 flex items-center justify-start pl-1 relative"
                      style={{ width: `${toNumber(data2020[index].male)}%` }}
                    ></div>
                  </div>
                </div>
